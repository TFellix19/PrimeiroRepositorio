...
export function Marketingdigital({ servicos }) {
  ...
  var servico = servicos?.servicos[0];
  const handleChange = e => {
    e.target.value = e.target.value.replace(/[^0-9.]/g, '');
    precosArray[precosArray.indexOf(parseInt(e.target.id)) + 1] = e.target.value;
  };
  ...
  function Getdata(subservico) {
    ...
    return (
      <>
        <div className="row">
          <div className="col-9">
            <p className="card-title">{subservico.nome}{" "}{"(ID" + " " + subservico.idsubservico + ")"}</p>
  ...
  function SendUpdate() {
    ...
    subservico.map(subservico => {
      ...
      updatedSubservico.preco = precosArray[precosArray.indexOf(parseInt(subservico.idsubservico)) + 1];

      api.put("/pedidos/updatepedido/" + subservico.idsubservico, updatedSubservico).then((data) => {
        if (data.status != "200") {
          success = false;
        }
      })
    ...
    <>
      ...
      <div class="card-header">Altere o preço de cada subservico</div>
        <div className="card-body">
          <img className="float-end" src={servico?.imagem} style={{ width: '200px', height: 'auto' }} alt="" />
          {servico?.subservicos.map((subservico) => Getdata(subservico))}
          <button className="btn btn-warning w-75 mt-3" onClick={SendUpdate}>Guardar preço </button>
        </div>
        <div class="card-footer">
          
        <button onClick={() => { setModalCriarShow(true) }} type="button" className="btn btn-success">Criar subserviço</button>
        <button onClick={() => { setModalEliminarShow(true) }} type="button" className="btn btn-outline-danger">Eliminar subserviço</button>
        <button onClick={() => { setModalEditarPackShow(true) }} type="button" className="btn btn-secondary float-end">Editar pack</button>
        ...
        <EditarPack
          show={modalEditarPackShow}
          onHide={() => setModalEditarPackShow(false)}
          subservico={
            subservico
          } />
